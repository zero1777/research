...
class Rockmate(torch.nn.Module):
    def __init__(
        ...
        print(f'dict_constants: {self.dict_constants}') # None
        print(f'init_code: {self.init_code}') # None
        print(f'output main target: {self.output.main_target}') # Output node name
    ...
    def get_sequence(self, budget):
        ...
        self.fwd_seq, self.bwd_seq = self.seq.cut_fwd_bwd() # RK_Sequence
        ...
        # op_sched : OpSchedule
        # print(f'fwd_op_sched_list: {self.fwd_op_sched_list}\n')
        # print(f'bwd_op_sched_list: {self.bwd_op_sched_list}\n')
        ...
        # print(f'fwd_op_list: {self.fwd_op_list}\n')
        # print(f'bwd_op_list: {self.bwd_op_list}\n')
    ...
    def forward(self, *args, record_mem=False, compiled=True, **kwargs):
        ...
        for k, v in model_inputs.items():
            ...
            print(f'k: {k}, v: {v}') # input 'x'
        exec(self.init_code, self.storage.gd, self.storage.ld)
        for kg in self.list_kg: # K_graph_list
            for kdn in kg.list_kdn: # data node
